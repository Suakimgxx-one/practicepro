...
class DynamicsComparisonResult:
    ...
    def flagged_regions(
        self, threshold_db: float = 4.0, max_gap_seconds: float = 1.0
        ...
        for point in self.points:
            ...
            if label is not None:
                breaks_region = (
                    ...
                    or (last_time is not None and point.reference_time - last_time > max_gap_seconds)
                )
                if breaks_region:
...
def compare_dynamics(
    reference: LoudnessContour,
    student: LoudnessContour,
    alignment: AlignmentResult,
    max_time_gap: float = 0.3,
    ...
    for t_ref, ref_loudness in zip(reference.times, reference.loudness_db):
        ...
        best_idx = min(
            candidate_indices, key=lambda i: abs(student_times_valid[i] - target_time)
        )
        ...
        points.append(
            DynamicsDeviationPoint(
                ...
                student_time=float(student_times_valid[best_idx]),
                reference_loudness_db=float(ref_loudness),
                student_loudness_db=student_loudness,
