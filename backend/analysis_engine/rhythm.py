...
def detect_onsets(waveform: np.ndarray, sr: int, hop_length: int = HOP_LENGTH) -> np.ndarray:
    onset_times = librosa.onset.onset_detect(
        y=waveform, sr=sr, hop_length=hop_length, units="time", backtrack=True
    )
...
class RhythmComparisonResult:
    ...
    def flagged_regions(
        self, threshold_seconds: float = 0.1, max_gap_seconds: float = 1.0
...
def compare_rhythm(
    ...
    student_onsets: np.ndarray,
    alignment: AlignmentResult,
    max_match_gap: float = 0.3,
    ...
    for t_ref in reference_onsets:
        ...
        points.append(
            RhythmDeviationPoint(
                reference_time=float(t_ref),
                predicted_student_time=float(predicted),
                actual_student_time=actual,
                timing_offset_seconds=actual - predicted,
            )
        )
    ...
    return RhythmComparisonResult(
        points=points,
        unmatched_reference_onsets=unmatched_reference,
        unmatched_student_onsets=unmatched_student,
    )
