...
class TempoComparisonResult:
    ...
    def flagged_regions(
        self, ratio_threshold: float = 0.15, max_gap_seconds: float = 1.0
...
def compute_tempo_ratio_curve(
    alignment: AlignmentResult, sample_interval: float = 0.25
) -> list[TempoRatioPoint]:
...
def compare_tempo(
    ...
    return TempoComparisonResult(
        points=points,
        reference_average_bpm=reference_bpm,
        student_average_bpm=student_bpm,
    )
