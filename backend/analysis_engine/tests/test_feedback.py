...
def _fake_response(text: str):
    """Mimics the shape of an anthropic.Anthropic().messages.create()
    response closely enough for generate_feedback's parsing logic —
    a content list of blocks with .type and .text."""
    return SimpleNamespace(content=[SimpleNamespace(type="text", text=text)])


def test_build_analysis_summary_includes_only_provided_categories():
    pitch_result = PitchComparisonResult(
        points=[
            PitchDeviationPoint(
                reference_time=1.0,
                student_time=1.0,
    ...
    assert "tempo" not in summary
    assert "dynamics" not in summary


def test_build_analysis_summary_shapes_flagged_regions_correctly():
    dynamics_result = DynamicsComparisonResult(
        points=[
            DynamicsDeviationPoint(
                reference_time=2.0,
                student_time=2.0,
                reference_loudness_db=-20.0,
                student_loudness_db=-10.0,
                loudness_difference_db=10.0,
            )
        ]
    )
    ...
    assert len(summary["dynamics"]["flagged_regions"]) == 1
...
def test_generate_feedback_parses_valid_response():
    ...
    client.messages.create.return_value = _fake_response(
        json.dumps(
            [
                {
                    "category": "pitch",
                    "text": "Sharp by 30 cents around 0:42.",
                    "timestamp_reference": 42.0,
                }
            ]
        )
    )
    ...
    assert result[0]["timestamp_reference"] == 42.0
