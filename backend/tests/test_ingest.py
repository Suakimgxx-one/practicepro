...
async def test_youtube_recording_processes_successfully(client, tmp_path):
    ...
    with patch("worker.tasks.ingest._download_audio", return_value=fake_audio):
        response = await client.post("/api/v1/recordings", json={"user_id": user_id, "type": "reference", "source": "youtube", "source_url": "https://youtube.com/watch?v=fake123"})
...
async def test_youtube_recording_marks_failed_on_download_error(client):
    user_id = await _make_user(client)
    with patch("worker.tasks.ingest._download_audio", side_effect=RuntimeError("Video unavailable")):
        response = await client.post("/api/v1/recordings", json={"user_id": user_id, "type": "reference", "source": "youtube", "source_url": "https://youtube.com/watch?v=badvideo"})
    recording_id = response.json()["id"]
    check = await client.get(f"/api/v1/recordings/{recording_id}")
...
async def test_youtube_recording_marks_failed_on_invalid_audio(client, tmp_path):
    ...
    with patch("worker.tasks.ingest._download_audio", return_value=fake_bad_file):
        response = await client.post("/api/v1/recordings", json={"user_id": user_id, "type": "reference", "source": "youtube", "source_url": "https://youtube.com/watch?v=corrupt"})
    recording_id = response.json()["id"]
    check = await client.get(f"/api/v1/recordings/{recording_id}")
    assert check.json()["status"] == "failed"
...
async def test_upload_source_recording_does_not_trigger_youtube_task(client):
    user_id = await _make_user(client)
    response = await client.post("/api/v1/recordings", json={"user_id": user_id, "type": "student", "source": "upload"})
