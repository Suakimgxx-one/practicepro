...
async def test_create_recording_youtube_requires_url(client):
    user_id = await _make_user(client)
    response = await client.post("/api/v1/recordings", json={"user_id": user_id, "type": "reference", "source": "youtube"})
...
async def test_list_recordings_for_user(client):
    user_id = await _make_user(client)
    await client.post("/api/v1/recordings", json={"user_id": user_id, "type": "student", "source": "upload"})
