...
def _make_wav_bytes(duration_seconds: float = 0.5, sample_rate: int = 16000) -> bytes:
    """Generates a short, valid sine-wave WAV file in memory — a real,
    decodable audio fixture without needing a checked-in binary file."""
...
async def test_upload_rejects_corrupt_audio(client):
    ...
    # Correct extension, garbage content — must fail the decode step,
    ...
    # The recording should be marked failed, not left pending.
