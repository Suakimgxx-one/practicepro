...
async def test_upload_valid_audio(client):
    ...
    response = await client.post(f"/api/v1/recordings/{recording_id}/upload", files={"file": ("test.wav", _make_wav_bytes(), "audio/wav")})
...
async def test_upload_rejects_corrupt_audio(client):
    _, recording_id = await _make_user_and_upload_recording(client)
    response = await client.post(f"/api/v1/recordings/{recording_id}/upload", files={"file": ("test.wav", b"this is not really a wav file" * 100, "audio/wav")})
...
async def test_upload_rejects_when_already_ready(client):
    ...
    first = await client.post(f"/api/v1/recordings/{recording_id}/upload", files={"file": ("test.wav", wav_bytes, "audio/wav")})
...
async def test_upload_to_nonexistent_recording(client):
    response = await client.post(f"/api/v1/recordings/{uuid.uuid4()}/upload", files={"file": ("test.wav", _make_wav_bytes(), "audio/wav")})
