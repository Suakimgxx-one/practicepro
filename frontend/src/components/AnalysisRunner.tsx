  ...
  const handleCompare = async () => {
    ...
    try {
      const created = await createAnalysisSession({
        user_id: userId,
        reference_recording_id: referenceRecordingId,
        student_recording_id: studentRecordingId,
      });
    ...
    return (
      <div className="pt-6 border-t border-line">
        <button
          ...
          disabled={starting}
          className="text-sm font-medium text-paper bg-ink hover:bg-brass-dark disabled:opacity-50 px-5 py-2.5 rounded transition-colors"
        >
          {starting ? "Starting" : "Compare this attempt"}
        </button>
        {startError && <p className="text-brick text-sm mt-2">{startError}</p>}
      </div>
    );
  }
    ...
    return (
      <div className="pt-6 border-t border-line">
        <p className="text-sm text-brick">
          Analysis failed. This can happen if the recordings don't share enough overlapping
          content for alignment to work, or if a step in the pipeline hit an error.
        </p>
      </div>
    );
  }
